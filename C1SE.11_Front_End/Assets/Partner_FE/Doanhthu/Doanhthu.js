...
//lấy dữ liệu cho mục doanh thu
let dataRevenue = [];
function printRevenue(){//------------>In ra danh sách cách vé
    const apiUrl = `http://localhost:3000/partner/revenue/${dataAccount.id}`;
    fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
        const tbody = document.querySelector(`.table tbody`);
        tbody.innerHTML = '';
        dataRevenue = data.data;
        console.log('Data: ', data);
        dataRevenue.forEach((e,index) => {
            ...
            newRow.innerHTML = `
                ...
                <td>${e.trip_name}</td>
                <td>${e.departure_datetime} - ${e.arrival_datetime}</td>
