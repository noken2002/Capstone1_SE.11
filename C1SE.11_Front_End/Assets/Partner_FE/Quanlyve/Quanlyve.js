...
let dataTicket = [];
function printTicket(){//------------>In ra danh sách cách vé
    const apiUrl = `http://localhost:3000/partner/ticket/${dataAccount.id}`;
    fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
        ...
        dataTicket.forEach((e,index) => {
            ...
            newRow.innerHTML = `
                ...
                <td>${e.sum_buy_ticket}/${e.sum_max_ticket}</td>`;
