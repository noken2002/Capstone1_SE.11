...
//lấy dữ liệu dòng tăng trưởng
let dataGrowthRate = [];
function printGrowthRate(){//------------>In ra danh sách cách vé
    const apiUrl = `http://localhost:3000/partner/growthRate/${dataAccount.id}`;
    fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
        const divRevenue = document.querySelector('#sum_revenue');
        const divPassenger = document.querySelector('#sum_passenger');
        divRevenue.innerHTML = '';
        divPassenger.innerHTML = '';
        dataManagerCoach = data.data;
        console.log('Data: ', data);
        dataManagerCoach.forEach((e) => {
            let code_color = '';
            let temp;
            temp = 100 * (e.total_revenue_current_year - e.total_revenue_previous_year)/e.total_revenue_previous_year;
            if(temp<0){
                code_color = 'D81919';
            } else{
                code_color = '34A521';
            ...
            divRevenue.innerHTML = `
                <img class="img-research" src="./public/images/coins.png"  alt="">
                <span>Tổng doanh thu so với năm trước</span>
                <span style="color:#7E7373;">24 giờ qua</span>
                <span style="color:#${code_color};">${Math.abs(temp).toFixed(2)}%</span>`;
            
            temp = 100 * (total_passenger_count_current_year - e.total_passenger_count_previous_year)/e.total_passenger_count_previous_year;
            if(temp<0){
                code_color = 'D81919';
            } else{
                code_color = '34A521';
            ...
            divPassenger.innerHTML = `
                <img class="img-research" src="./public/images/standing-man.png"  alt="">
                <span>Tổng doanh thu so với năm trước</span>
                <span style="color:#7E7373;">24 giờ qua</span>
                <span style="color:#${code_color};">${Math.abs(temp).toFixed(2)}%</span>`;
        })
